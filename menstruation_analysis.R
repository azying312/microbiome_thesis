...
# vaginal_samples <- read.csv("/Users/alicezhang/Desktop/microbiome_data/cleaned_data/cleaned_vaginal_samples_data.csv", header=TRUE)
survey_data <- read.csv("/Users/alicezhang/Desktop/microbiome_data/alice_cleaned_survey_data.csv", header=TRUE)

# full_data <- vaginal_samples %>% 
#   full_join(full_menstruation_data)

## Q: What proportion of self-reported days had blood; days that are inUminn & in self report
selfReport_blood <- full_data %>% 
  filter(inUminn==TRUE & inSelfReport==TRUE & uMinn_menstruation==1)
...
## Q: What proportion of self reported have corresponding samples?
selfReport_blood <- full_data %>% 
  filter(inUminn==TRUE & inSelfReport==TRUE)
dim(selfReport_blood)
sum((full_data$inSelfReport==TRUE), na.rm=TRUE) # 148
...
## Q: What proportion of self reported no menstruation had samples
dim(full_data %>% filter(inSelfReport==1 & menstruation==0 & inUminn==TRUE))
...
dim(selfReport_samples) # 87 with both self report & samples
...
full_data <- full_data %>% 
  mutate(person_menstruating=ifelse((menstruation_status==1 | menstruation_status==2 | menstruation_status==3 | menstruation_status==7), 1, 
                                    ifelse(menstruation_status==8, -1, 0)))

#### Contingency Tables

# check people who said they don't menstruate, who entered on the menses data
survey_data <- survey_data %>% 
  select(biome_id, menstruate) %>% 
  rename(survey_menstruate=menstruate)
# new col: 1 if there's an entry on menstruation data, 0 if no entry
...
mismatched_entries <- survey_data %>%
  filter(menstruate == 0 & menses_data_entry == 1)
dim(mismatched_entries)
unique(mismatched_entries$biome_id)
...
missing_entries <- survey_data %>%
  filter(menstruate == 1 & menses_data_entry == 0)
dim(missing_entries)
length(unique(missing_entries$biome_id))

# Number of people who don't menstruate
no_menses <- survey_data %>%
  filter(menstruate == 0 & menses_data_entry == 0)
dim(no_menses)
...
yes_menses <- survey_data %>%
  filter(menstruate == 1 & menses_data_entry == 1)
dim(yes_menses)
(unique(yes_menses$biome_id))
length((unique(yes_menses$biome_id)))

## Contingency table
menses_cont_table <- table(survey_data$menses_data_entry, survey_data$menstruate) # menstruate is from the volunteer survey
...
### Color by what filled in the day
...
## All days plot (NA for value menas not in data at all)
...
### Plot 1: Map to with menses relative day 1
full_data_subset <- filtered_data %>% 
  select(biome_id, logDate, menstruation_status, uMinn_menstruation, menstruation, person_menstruating)
complete_grid <- expand.grid(biome_id = unique(full_data_subset$biome_id),
                             logDate = all_days)
...
### Plot 2: Map to impute from Uminn sample & menses relative day 1
